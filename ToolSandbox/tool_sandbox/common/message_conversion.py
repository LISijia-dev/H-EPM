...
class Message:
    ...
    # Optional field for storing entropy data
    entropy_data: Optional[str] = None
...
def to_openai_messages(
    ...
    list[
        dict[
            Literal["role", "content", "tool_call_id", "name", "tool_calls", "entropy_data"],
            Any,
        ]
    ],
    list[list[int]],
    ...
    for i, message in enumerate(messages):
        ...
        if message.sender == RoleType.SYSTEM and message.recipient == RoleType.AGENT:
            openai_message = {"role": "system", "content": message.content}
            if message.entropy_data:
                openai_message["entropy_data"] = message.entropy_data
            openai_messages.append(openai_message)
        elif message.sender == RoleType.USER and message.recipient == RoleType.AGENT:
            openai_message = {"role": "user", "content": message.content}
            if message.entropy_data:
                openai_message["entropy_data"] = message.entropy_data
            openai_messages.append(openai_message)
        elif (
            ...
            # Add entropy data to the assistant message if available
            if message.entropy_data:
                openai_messages[-1]["entropy_data"] = message.entropy_data
        elif message.sender == RoleType.AGENT and message.recipient == RoleType.USER:
            openai_message = {"role": "assistant", "content": message.content}
            if message.entropy_data:
                openai_message["entropy_data"] = message.entropy_data
            openai_messages.append(openai_message)
        else:
