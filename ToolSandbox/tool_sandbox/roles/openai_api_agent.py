...
def get_openai_model(model_name: str, base_url: str = "http://localhost:8000/v1"):
    """OpenAI-compatible client for local/vLLM Qwen and similar servers."""
    client = OpenAI(
        base_url=base_url,
...
class OpenAIAPIAgent(BaseRole):
    ...
    def __init__(self, tool_graph_path: Optional[str] = None, enable_tool_suggestion: bool = True) -> None:
        # 初始化工具图相关属性
        self.tool_graph_path = "/home/v-sijiali/project_s/tr-sandbox/tau2_graph_4_1.json"
        ...
        # Sandbox tool name <-> tool graph node (tau2_graph.json nodes) via description similarity
        self._tool_node_mapping_fingerprint: Optional[Any] = None
        self._sandbox_to_graph_node: Dict[str, str] = {}
        self._graph_node_to_sandbox: Dict[str, str] = {}
        ...
        elif hasattr(self, "model_name") and "qwen" in self.model_name.lower():
            # Local or remote OpenAI-compatible API for Qwen3 (e.g. vLLM). Override URL via QWEN_BASE_URL.
            qwen_base = os.environ.get("QWEN_BASE_URL", "http://localhost:8000/v1")
            self.openai_client, self.deployment_name = get_openai_model(self.model_name, base_url=qwen_base)
        else:
    ...
    def _is_qwen_model(self) -> bool:
        return "qwen" in getattr(self, "model_name", "").lower()
    ...
    def respond(self, ending_index: Optional[int] = None) -> None:
        ...
        # Qwen3 may emit tool calls as inline <tool_call>...</tool_call> in content instead of tool_calls.
        if (
            (openai_response_message.tool_calls is None or len(openai_response_message.tool_calls) == 0)
            and isinstance(openai_response_message.content, str)
            and "<tool_call>" in openai_response_message.content
        ):
            inline_tool_calls = []
            cleaned_content = openai_response_message.content
            pattern = re.compile(r"<tool_call>\s*(\{[\s\S]*?\})\s*</tool_call>", re.IGNORECASE)
            for match in pattern.finditer(openai_response_message.content):
                ...
                try:
                    ...
                    if name:
                        ...
                        tool_call_obj.id = f"call_{uuid.uuid4().hex[:24]}"
                        tool_call_obj.function = SimpleNamespace()
                        tool_call_obj.function.name = name
                        tool_call_obj.function.arguments = json.dumps(arguments)
                        inline_tool_calls.append(tool_call_obj)
                except Exception as e:
                    logging.getLogger(__name__).warning("Failed to parse tool_call block: %s", e)

            if inline_tool_calls:
                openai_response_message.tool_calls = inline_tool_calls  # type: ignore[assignment]
                cleaned_content = pattern.sub("", cleaned_content).strip()
                openai_response_message.content = cleaned_content if cleaned_content else None

        # Message contains no tool call, aka addressed to user
        if openai_response_message.tool_calls is None or len(openai_response_message.tool_calls) == 0:
            if openai_response_message.content is not None:
                response_messages = [
                    Message(
                        sender=self.role_type,
            ...
            # Tool graph online updates disabled (previously called update_graph_from_messages here).
    ...
    def model_inference(
        ...
        with all_logging_disabled():
            # Use deployment name for Azure models, otherwise use model name
            model_to_use = self.deployment_name if self.deployment_name else self.model_name
            is_qwen = self._is_qwen_model()
            try:
                if is_qwen:
                    # Qwen3 (vLLM / OpenAI-compatible): disable thinking in chat template; tool_choice when tools sent
    ...
    def _ensure_tool_node_mapping(self) -> None:
        """Align each sandbox tool (agent-facing name) to exactly one graph node via description similarity."""
        if not self.tool_graph_path:
            return
        try:
            ...
            return
        fp = frozenset(name_to_tool.keys())
        if fp == self._tool_node_mapping_fingerprint:
            return
        self._rebuild_tool_node_mapping(name_to_tool)
        self._tool_node_mapping_fingerprint = fp

    def _rebuild_tool_node_mapping(self, name_to_tool: Dict[str, Callable[..., Any]]) -> None:
        """One-to-one: greedy assignment on (similarity, tool, node) pairs sorted by score."""
        self._sandbox_to_graph_node = {}
        self._graph_node_to_sandbox = {}
        graph = self._load_tool_graph()
        nodes = list(graph.get("nodes") or [])
        ndesc = graph.get("node_descriptions") or {}
        if not nodes:
            return
        node_texts = {n: f"{n}. {ndesc.get(n, '')}".strip() for n in nodes}
        tool_texts: Dict[str, str] = {}
        for name, tool in name_to_tool.items():
            try:
                oai = convert_to_openai_tool(tool, name)
                desc = (oai.get("function") or {}).get("description") or ""
                tool_texts[name] = f"{name}. {desc}".strip()
            except Exception as e:
                print(f"[TOOL_GRAPH] convert_to_openai_tool failed for {name}: {e}")
                tool_texts[name] = name
        pairs: List[Tuple[float, str, str]] = []
        for tname, ttext in tool_texts.items():
            for n in nodes:
                sim = self._calculate_similarity(ttext, node_texts[n])
                if tname == n:
                    sim = 1.0
                pairs.append((sim, tname, n))
        pairs.sort(key=lambda x: (-x[0], 0 if x[1] == x[2] else 1))
        used_tool: set[str] = set()
        used_node: set[str] = set()
        for sim, tname, n in pairs:
            if tname in used_tool or n in used_node:
                continue
            self._sandbox_to_graph_node[tname] = n
            self._graph_node_to_sandbox[n] = tname
            used_tool.add(tname)
            used_node.add(n)
        graph_node_set = set(nodes)
        for tname in tool_texts:
            if tname not in used_tool and tname in graph_node_set and tname not in used_node:
                self._sandbox_to_graph_node[tname] = tname
                self._graph_node_to_sandbox[tname] = tname
                used_tool.add(tname)
                used_node.add(tname)
        print(
            f"[TOOL_GRAPH] sandbox↔node mapping: {len(self._sandbox_to_graph_node)} tools aligned "
            f"({len(nodes)} graph nodes)."
        )

    def _graph_node_for_sandbox_tool(self, sandbox_tool: str) -> Optional[str]:
        """Resolve agent-facing tool name to tool-graph node id (for edge `u`)."""
        self._ensure_tool_node_mapping()
        graph = self._load_tool_graph()
        nodes = set(graph.get("nodes") or [])
        if sandbox_tool in self._sandbox_to_graph_node:
            return self._sandbox_to_graph_node[sandbox_tool]
        if sandbox_tool in nodes:
            return sandbox_tool
        return None

    def _sandbox_for_graph_node(self, graph_node: str, available: Optional[set] = None) -> str:
        """Graph node `v` -> sandbox tool name the agent can call."""
        self._ensure_tool_node_mapping()
        sb = self._graph_node_to_sandbox.get(graph_node, graph_node)
        if available is not None:
            if sb in available:
                return sb
            if graph_node in available:
                return graph_node
            return sb
        return sb

    def _get_connected_tools(self, current_tool: str) -> List[Dict[str, Any]]:
        """Outgoing edges from the graph node mapped from the current sandbox tool."""
        graph_u = self._graph_node_for_sandbox_tool(current_tool)
        if graph_u is None:
            print(f"[TOOL_GRAPH] no graph node for sandbox tool: {current_tool}")
            return []
        graph = self._load_tool_graph()
        connected_tools: List[Dict[str, Any]] = []
        for edge in graph.get("edges", []):
            try:
                if edge.get("u") == graph_u and edge.get("v"):
                    v = edge.get("v")
                    connected_tools.append(
                        {
                            "tool": v,
                            "sandbox_tool": self._graph_node_to_sandbox.get(v, v),
...
Please only answer "Yes" or "No", do not explain the reason."""
            ...
            if '5.1' in model_to_use:
                ...
                # No tool_choice here: OpenAI-compatible servers require `tools` whenever tool_choice is set.
                response = self.openai_client.chat.completions.create(
                    model=model_to_use,
                    messages=messages,
                    max_tokens=10,
    ...
    def _suggest_next_tools(self, current_tool: str, conversation_context: str, 
        ...
        avail_set = set(available_tools) if available_tools else None
        if avail_set is not None:
            connected_tools = [
                t
                for t in connected_tools
                if (t.get("sandbox_tool") or t["tool"]) in avail_set or t["tool"] in avail_set
            ]
        ...
        for tool_info in connected_tools:
            graph_node = tool_info["tool"]
            sandbox_tool = tool_info.get("sandbox_tool") or self._sandbox_for_graph_node(graph_node, avail_set)
            ...
            if should_use_summary and current_infos and len(current_infos) > 0:
                ...
                for info in current_infos:
                    ...
                    print(f"[TOOL_SUGGESTION] Comparing context with info for graph node {graph_node}: {info}")
                    print(f"[TOOL_SUGGESTION] Calculated similarity between context and info for graph node {graph_node}: {similarity}")
    ...
    def _format_decision_rules(self, tool_name: str) -> str:
        ...
        rules_key = tool_name
        if self.tool_graph_path:
            g = self._graph_node_for_sandbox_tool(tool_name)
            if g is not None:
                rules_key = g
        
        decision_rules = []
        if hasattr(self, 'node_rules') and self.node_rules and rules_key in self.node_rules:
            rules = self.node_rules[rules_key].get('decision_rules', [])
...
class GPT_4o_mini_Agent(OpenAIAPIAgent):
