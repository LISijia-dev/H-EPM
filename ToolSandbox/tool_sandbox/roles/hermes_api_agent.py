...
def to_chat_completion_message(choice: CompletionChoice) -> ChatCompletionMessage:
    ...
    while True:
        ...
        if i2 == -1:
            # Try to find the end token on the next line if not found
            next_line_start = text.find('\n', start)
            if next_line_start != -1:
                i2 = text.find(tce_token, next_line_start)
            if i2 == -1:
                print(f"[WARNING] No end token {tce_token} found for tool call starting at position {start}")
                break
        assert i2 != -1
        cur = i2 + len(tce_token)
        try:
            tool_call_text = text[start:i2].strip()
            print(f"[DEBUG] Parsing tool call: {tool_call_text}")
            tool_calls.append(json.loads(tool_call_text))
        except json.JSONDecodeError as e:
            print(f"[WARNING] Failed to parse tool call as JSON: '{text[start:i2]}' - {e}")
            # Try to continue with other tool calls instead of failing completely
            continue
    content = ""
...
class HermesAPIAgent(BaseRole):
    ...
    def respond(self, ending_index: Optional[int] = None) -> None:
        ...
        print(f"[DEBUG] Raw Hermes response: {hermes_response.choices[0].text}")
        
        # Parse response
        openai_response = completion_to_chat_completion(hermes_response)
        openai_response_message = openai_response.choices[0].message
        
        # Debug: Print parsed tool calls
        if openai_response_message.tool_calls:
            print(f"[DEBUG] Parsed tool calls: {[tc.function.name for tc in openai_response_message.tool_calls]}")
        else:
            print("[DEBUG] No tool calls found in response")
